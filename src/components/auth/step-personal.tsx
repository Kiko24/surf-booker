...
import type { SignupInput, SignupResult } from "@/lib/auth/signup";
...
type Props = {
  ...
  signupAction: (input: SignupInput) => Promise<SignupResult>;
};
...
export function StepPersonal({ email, onSuccess, onEmailConflict, signupAction }: Props) {
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
        ...
        <label className="flex items-start gap-2 text-sm text-text-secondary cursor-pointer">
          <input
            ...
            className="mt-[1px] h-4 w-4 accent-accent cursor-pointer"
          />
          <span>
        ...
        {errors.acceptedTerms && (
          <p className="text-xs text-error -mt-2">{errors.acceptedTerms}</p>
        )}

        {generalError && (
          <p className="text-sm text-error text-center">{generalError}</p>
        )}
